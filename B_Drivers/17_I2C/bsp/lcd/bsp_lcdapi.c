...
void lcd_drawline(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2)
{
	...
	delta_x = x2 - x1; 					/* Total x-axis */
	delta_y = y2 - y1; 					/* Total y-axis */
	uRow = x1; 
	uCol = y1; 
	if(delta_x > 0) 					/* 设置单步方向 */ 
		incx = 1;
	...
	{  
		...
		lcd_drawpoint(uRow, uCol, tftlcd_dev.forecolour);/* 画点 */
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void lcd_draw_Circle(unsigned short x0,unsigned short y0,unsigned char r)
{
    ...
    {
        lcd_drawpoint(x  + mx, y  + my, tftlcd_dev.forecolour);
        lcd_drawpoint(y  + mx, x  + my, tftlcd_dev.forecolour);
        lcd_drawpoint(-x + mx, y  + my, tftlcd_dev.forecolour);
        lcd_drawpoint(-y + mx, x  + my, tftlcd_dev.forecolour);
...
{  							  
	...
	{   
		...
		for(t1 = 0; t1 < 8; t1++)
		{			    
			if(temp & 0x80)lcd_drawpoint(x, y, tftlcd_dev.forecolour);
			else if(mode==0)lcd_drawpoint(x, y, tftlcd_dev.backcolour);
			temp <<= 1;
			y++;
